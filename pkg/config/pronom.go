...
func Droid() string {
	if pronom.droid == "" {
		...
		return filepath.Join(xfilter.home, droid)
	}
	if filepath.Dir(pronom.droid) == "." {
		return filepath.Join(xfilter.home, pronom.droid)
	}
	return pronom.droid
}
...
func Container() string {
	if pronom.container == "" {
		...
		return filepath.Join(xfilter.home, container)
	}
	if filepath.Dir(pronom.container) == "." {
		return filepath.Join(xfilter.home, pronom.container)
	}
...
func latest(prefix, suffix string) (string, error) {
	var hits []string
	var ids []int
	files, err := ioutil.ReadDir(xfilter.home)
	if err != nil {
		return "", err
	}
	for _, f := range files {
	...
	if len(hits) == 0 {
		return "", fmt.Errorf("Config: no file in %s with prefix %s", xfilter.home, prefix)
	}
	if len(hits) == 1 {
		return hits[0], nil
	}
...
func Reports() string {
	...
	return filepath.Join(xfilter.home, pronom.reports)
}
